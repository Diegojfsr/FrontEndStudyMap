...
Rota de login:
- Recebe as credenciais e, se válidas, cria um token com informações do usuário.
- O token é enviado como resposta.
Middleware de validação:
- Verifica se o token foi enviado e se é válido.
- Caso válido, permite o acesso às rotas protegidas.
Rota protegida:
- Só pode ser acessada se o token for válido.
- 
Agora você tem uma implementação básica de autenticação com JWT.
